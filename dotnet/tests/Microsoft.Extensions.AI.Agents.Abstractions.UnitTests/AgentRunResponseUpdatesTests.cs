...
public class AgentRunResponseUpdateTests
{
    ...
    {
        ModelResponseUpdate chatResponseUpdate = new()
        {
            AdditionalProperties = new Dictionary<string, object?>(),
            AuthorName = "author",
            Contents = [new TextModelContent("hello")],
    ...
    public void PropertiesRoundtrip()
    {
        ...
        Assert.Null(update.Role);
        update.Role = ModelRole.Assistant;
        Assert.Equal(ModelRole.Assistant, update.Role);

        Assert.Empty(update.Contents);
        update.Contents.Add(new TextModelContent("text"));
        Assert.Single(update.Contents);
        Assert.Equal("text", update.Text);
        Assert.Same(update.Contents, update.Contents);
        IList<ModelContent> newList = [new TextModelContent("text")];
    ...
    [Fact]
    public void TextGetUsesAllTextModelContent()
    {
        AgentRunResponseUpdate update = new()
        {
            Role = ModelRole.User,
            ...
            [
                ...
                new TextModelContent("text-1"),
                new TextModelContent("text-2"),
                //new FunctionResultContent("callId1", "result"),
            ],
        };

        TextModelContent textContent = Assert.IsType<TextModelContent>(update.Contents[3]);
        Assert.Equal("text-1", textContent.Text);
        Assert.Equal("text-1text-2", update.Text);
        Assert.Equal("text-1text-2", update.ToString());

        ((TextModelContent)update.Contents[3]).Text = "text-3";
    ...
    public void JsonSerializationRoundtrips()
    {
        ...
        Assert.IsType<TextModelContent>(result.Contents[0]);
        Assert.Equal("text-1", ((TextModelContent)result.Contents[0]).Text);
        ...
        Assert.IsType<TextModelContent>(result.Contents[4]);
        Assert.Equal("text-2", ((TextModelContent)result.Contents[4]).Text);

        Assert.Equal("author", result.AuthorName);
        Assert.Equal(ModelRole.Assistant, result.Role);
